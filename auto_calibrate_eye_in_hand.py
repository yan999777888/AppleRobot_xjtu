...
#
SAVE_DIR = os.path.join(curPath, "calibration_results")
...
def compute_consistency_error(R_g2b_list, t_g2b_list,
                              R_t2c_list, t_t2c_list, T_cam2gripper):
    """
    T_base_target = T_base_gripper @ T_cam2gripper @ T_cam_target
    """
    target_positions = []
    for i in range(len(R_g2b_list)):
        ...
        T_bt = T_bg @ T_cam2gripper @ T_ct
        target_positions.append(T_bt[:3, 3])

    target_positions = np.array(target_positions)
    mean_pos = np.mean(target_positions, axis=0)
    errors = [np.linalg.norm(p - mean_pos) for p in target_positions]
...
def main():
    ...
    print("=" * 60)
    print("SR4 + RealSense D405 (Eye-in-Hand)")
    print("=" * 60)

    # ---- 1.  ----
    print("\n[1/5] RealSense D405...")
    camera = RealSenseD405()
    camera.start()
    ...
    user_abort = False
    collision_count = 0
    reference_vis = None    # last accepted detection image for comparison
    ...
    for i, pose in enumerate(poses):
        ...
        for _ in range(NUM_CAPTURES):
            ...
            if found:
                ...
                cv2.imshow("Eye-in-Hand Calibration", color_img)
            cv2.waitKey(1)
            time.sleep(0.1)
    ...
    np.savetxt(os.path.join(SAVE_DIR, "dist_coeffs.txt"), camera.dist_coeffs)
    ...
    for name, method in methods.items():
        try:
            ...
            ortho = np.linalg.norm(R_c2g @ R_c2g.T - np.eye(3))

            errors, _ = compute_consistency_error(
                R_gripper2base_list, t_gripper2base_list,
                R_target2cam_list, t_target2cam_list, T_c2g
            )
            ...
            all_results[name] = T_c2g
            np.savetxt(os.path.join(SAVE_DIR, f"T_cam2gripper_{name}.txt"), T_c2g)
    ...
    T_cam2gripper = all_results.get(primary_name, all_results.get(best_name))
    T_gripper2cam = np.linalg.inv(T_cam2gripper)

    np.savetxt(os.path.join(SAVE_DIR, "T_cam2gripper.txt"), T_cam2gripper)
    np.savetxt(os.path.join(SAVE_DIR, "T_gripper2cam.txt"), T_gripper2cam)
    ...
    errors, mean_target_pos = compute_consistency_error(
        R_gripper2base_list, t_gripper2base_list,
        R_target2cam_list, t_target2cam_list, T_cam2gripper
    )
    print(f"\n  :")
    ...
    rpy = Rotation.from_matrix(T_cam2gripper[:3, :3]).as_euler('xyz', degrees=True)
    t = T_cam2gripper[:3, 3] * 1000
    print(f"\n  T_cam2gripper:")
...
def use_calibration_result(camera_point, T_cam2gripper, T_base_gripper):
    """
    (Eye-in-Hand)

    Args:
        camera_point: [x, y, z] ()
        T_cam2gripper: 4x4 ->  (T_cam2gripper.txt)
        T_base_gripper: 4x4 ->  (flangePos)

    Returns:
        [x, y, z]
    """
    p_cam = np.array([camera_point[0], camera_point[1], camera_point[2], 1.0])
    p_base = T_base_gripper @ T_cam2gripper @ p_cam
    return p_base[:3]


def load_calibration(save_dir=SAVE_DIR):
    """
    Returns: T_cam2gripper (4x4 numpy array)
    """
    path = os.path.join(save_dir, "T_cam2gripper.txt")
