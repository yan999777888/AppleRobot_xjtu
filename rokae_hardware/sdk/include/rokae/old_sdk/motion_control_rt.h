 ...
 */
 class BaseMotionControl : public Base<BaseMotionControl> {
  public:
 ...
 template <>
 class ROKAE_EXPORT MotionControl<MotionControlMode::RtCommand> : public BaseMotionControl {
  public:
   MotionControl(std::shared_ptr<XService> sdkRpc, std::string remoteIP, int port);
   ~MotionControl() noexcept override;
   ...
   /**
    * @brief 使用周期调度，设置回调函数, 在回调函数中计算并发送运动命令
    * @param[in] callback 回调函数
    * @param[in] priority 任务优先级, 0为不指定。此参数仅当使用实时操作系统时生效，若无法设置会打印控制台错误信息。
    */
   void setControlLoop(const std::function<void(void)>& callback, int priority = 0);

   /**
    * @brief 开始周期性执行调度任务
    * @param[in] blocking 是否阻塞调用此函数的线程。无论阻塞与否，均可通过stopLoop()停止调度任务。
    ...
    * @note 另外，JointPosition/CartesianPose/Torque指令可通过setFinished()标识一次运动循环结束，标识后会机器人停止运动，
    * 呈现的效果和调用stopMove()一样。
  ...
  */
 template <unsigned short DoF>
 class ROKAE_EXPORT RtMotionControl : public MotionControl<MotionControlMode::RtCommand> {
  public:
   RtMotionControl(std::shared_ptr<XService> sdkRpc, const std::string& remoteIP, int port);

  // **********************************************************************
  // ********************           发送运动指令          *******************
   /**
    * @brief 发送JointPosition/CartesianPose/Torque命令
    * @note 开始运动后，持续调用此函数发送运动命令。由于目前控制周期为1ms，发送命令的周期也尽可能达到1ms。
    ...
    * xMateErPro机型最大值为 { 3000, 3000, 3000, 3000, 300, 300, 300 }
    * 其他机型最大值为 { 3000, 3000, 3000, 300, 300, 300 }
    * @param[out] ec 错误码
    */
    ...
    * @param[in] factor 笛卡尔空间阻抗控制系数, 最大值为 { 3000, 3000, 3000, 300, 300, 300 }, 单位: N/m, Nm/rad
    * @param[out] ec 错误码
    */
    ...
    * @throw RealtimeMotionException 机器人运动过程中发生错误
    */
   void MoveL(double speed, CartesianPose& start, CartesianPose& target);
    ...
    * @throw RealtimeMotionException 机器人运动过程中发生错误
    */
   void MoveC(double speed, CartesianPose& start, CartesianPose& aux, CartesianPose& target);

#ifdef XMATEMODEL_LIB_SUPPORTED
   /**
    * @brief MoveL指令，上位机规划路径，在到达target之前处于处于阻塞状态。如果运动中发生错误将停止阻塞状态并返回。
    * XMateModel模型会根据机器人实时轴角度值，计算每个规划位姿是否可达，如果遇到奇异点或超出工作空间，将提前退出。需要设置"jointPos_m"为接收数据。
    * @param[in] speed 速度比例系数, 范围 0 - 1
    * @param[in] start 起始位姿, 需要是机器人当前位姿，否则可能造成下电。如果设置了TCP，那么应该是工具相对于基坐标系的位姿。
    * @param[in] target 机器人目标位姿。同理如果设置了TCP，应是TCP相对于基坐标系的位姿
    * @param model xMate模型类，通过robot.model()获取
    * @throw RealtimeParameterException 起始或目标位姿参数错误
    */
   void MoveL(double speed, CartesianPose& start, CartesianPose& target, XMateModel<DoF>& model);
#endif
