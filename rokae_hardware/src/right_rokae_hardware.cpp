...
int main(int argc, char **argv){
    ...
    ros::init(argc, argv, "right_rokae_hardware_interface_node");
    ...
    if (!nh.getParam("/right_rokae_hardware_interface/joints", joint_names_))
    {
    ...
    if (!nh.getParam("/right_rokae_hardware/robot_ip", robot_ip_))
    {
        ROS_ERROR("No right_robot_ip found on param server");
        return false;
    }

    if (!nh.getParam("/right_rokae_hardware/local_ip", local_ip_))
    {
        ROS_ERROR("No right_robot_ip found on param server");
        return false;
    }
    ...
    if (!rokae_hardware_interface.init(nh, robot_ip_, local_ip_, joint_names_)) {
        ROS_ERROR("Failed to initialize right rokae Hardware Interface.");
    }
