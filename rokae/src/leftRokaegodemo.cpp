...
int main(int argc, char *argv[]) {
  setlocale(LC_ALL,"");
  ros::init(argc,argv,"pick_left");
  ros::NodeHandle nh;

  ros::Publisher pub_serial = nh.advertise<comm::serialData>("ap_robot/serial",10);
  comm::serialData leftClosedata, leftOpendata;//确保脉冲数的大小正确
  leftClosedata.data = {0xaa, 0x55, 0x02, 0x00, 0x03, 0x84, 0x50, 0x05, 0x27, 0x10, 0x3c, 0x01, 0x02, 0x03, 0x0d, 0x0a};
  leftOpendata.data = {0xaa, 0x55, 0x02, 0x01, 0x03, 0x84, 0x4e, 0x05, 0xd8, 0xf0, 0xcd, 0x01, 0x02, 0x03, 0x0d, 0x0a};
  ...
  try {
    ...
    std::string id;
    // float a=1.22173;
    // MoveJCommand moveJ_gohome({0.17, -0.14, 0.38, M_PI, 0, -M_PI_2});//移动到chushi姿态
    // MoveJCommand moveJ_releaseCartpos({0.4, -0.20, 0.0, -M_PI, 0, -M_PI});//移动到释放苹果姿态
    // MoveJCommand moveJ_godemoline({0.88,-0.12,0.45,-M_PI_2,M_PI_70d,-M_PI_2});//caizhaizitai
    MoveJCommand moveJ_gohome({0.28, 0.47, 0.084, 157*M_PI_1, -78*M_PI_1, 15*M_PI_1});//移动到chushi姿态
    MoveJCommand moveJ_releaseCartpos({0.06, 0.27, -0.26, -M_PI, -8*M_PI_1, 3*M_PI_1});//移动到释放苹果姿态
    MoveJCommand moveJ_godemoline({0.89,0.26,-0.02,103*M_PI_1,-83*M_PI_1,78*M_PI_1});//caizhaizitai
    MoveJCommand tran2({0.088,0.268,-0.284,-M_PI,5*M_PI_1,0});
    MoveJCommand tran1({0.22,0.268,-0.2,-M_PI,-36*M_PI_1,0});
    // robot.moveReset(ec);
    ...
    std::cout << " 闭合左手爪！" << std::endl;
    pub_serial.publish(leftClosedata); //闭合手爪
    ros::Duration(2).sleep(); // 等待2s

    robot.moveReset(ec);
    robot.moveAppend({tran2},id,ec);
    robot.moveStart(ec);
    waitForFinish(robot, id, 1);

    std::cout << " 打开左手爪！" << std::endl;
    pub_serial.publish(leftOpendata); //打开手爪
