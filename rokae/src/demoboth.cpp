    ...
    bool demogoline(const std::string &ip, int /*i*/) {
        try {
            if (ip == Rightip) {
                ...
                rbot_.moveAppend({moveJ_godemoline}, traj_id, ec);
                rbot_.moveStart(ec);
                ...
                MoveJCommand moveJ_gohome({0.22,0.268,-0.2,-179*M_PI_1,-36*M_PI_1,0});
                 std::error_code ec;
                std::string traj_id;
                lbot_.moveReset(ec);
                lbot_.moveAppend({moveJ_gohome}, traj_id, ec);
                lbot_.moveAppend({moveJ_godemoline}, traj_id, ec);
                lbot_.moveStart(ec);
...
int main(int argc, char *argv[]) 
{
    ...
    rightClosedata.data = {0xaa, 0x55, 0x02, 0x01,  0x08, 0x34, 0x50, 0x05, 0x27, 0x10, 0x3c, 0x01, 0x02, 0x03, 0x0d, 0x0a};
    rightOpendata.data  = {0xaa, 0x55, 0x02, 0x00,  0x08, 0x34, 0x4e, 0x05, 0xd8, 0xf0, 0xcd, 0x01, 0x02, 0x03, 0x0d, 0x0a};
    leftClosedata.data  = {0xaa, 0x55, 0x03, 0x00, 0x08, 0x34, 0x50, 0x05, 0x27, 0x10, 0x3c, 0x01, 0x02, 0x03, 0x0d, 0x0a};
    leftOpendata.data   = {0xaa, 0x55, 0x03, 0x01, 0x08, 0x34, 0x4e, 0x05, 0xd8, 0xf0, 0xcd, 0x01, 0x02, 0x03, 0x0d, 0x0a};
    ...
    for (int i = 0; i < loops && ros::ok(); ++i) {
        ...
        // ----- 第一次动作：右臂单独操作 -----
        ROS_INFO("[main] First action: Right arm only");
        ...
        for (int k = 0; k < 30 && ros::ok(); ++k) { ros::spinOnce(); std::this_thread::sleep_for(50ms); }
        ...
        // ----- 第二次动作：左臂单独操作 -----
        ROS_INFO("[main] Second action: Left arm only");
        ...
        // ----- 第三次动作：左右臂同步操作 -----
        ROS_INFO("[main] Third action: Both arms simultaneously");

        // 左右臂同时移动到采摘位（使用线程并行）
        std::atomic<bool> right_done{false}, left_done{false};
        std::thread right_thread([&]() {
            ROS_INFO("[main] Right arm: demogoline (parallel)");
            right_done = demo.demogoline(Rightip, i);
            if (!right_done) ROS_ERROR("[main] Right demogoline failed in parallel");
        });
        std::thread left_thread([&]() {
            ROS_INFO("[main] Left arm: demogoline (parallel)");
            left_done = demo.demogoline(Leftip, i);
            if (!left_done) ROS_ERROR("[main] Left demogoline failed in parallel");
        });
        ...
        if (!right_done || !left_done) {
            ROS_ERROR("[main] One or both arms failed to reach picking position");
            continue;
        }

        // 双手爪同时闭合
        ROS_INFO("[main] publish both hands close");
        pub_serial.publish(rightClosedata);
        pub_serial.publish(leftClosedata);
        for (int k = 0; k < 30 && ros::ok(); ++k) { ros::spinOnce(); std::this_thread::sleep_for(50ms); }
        ...
        right_done = false;
        left_done = false;
        std::thread right_release_thread([&]() {
            ROS_INFO("[main] Right arm: demogorelese (parallel)");
            right_done = demo.demogorelese(Rightip, i);
            if (!right_done) ROS_ERROR("[main] Right demogorelese failed in parallel");
        });
        std::thread left_release_thread([&]() {
            ROS_INFO("[main] Left arm: demogorelese (parallel)");
            left_done = demo.demogorelese(Leftip, i);
            if (!left_done) ROS_ERROR("[main] Left demogorelese failed in parallel");
        });
        ...
        if (!right_done || !left_done) {
            ROS_ERROR("[main] One or both arms failed to release");
            continue;
        }

        // 双手爪同时打开
        ROS_INFO("[main] publish both hands open");
        pub_serial.publish(rightOpendata);
        pub_serial.publish(leftOpendata);
        for (int k = 0; k < 30 && ros::ok(); ++k) { ros::spinOnce(); std::this_thread::sleep_for(50ms); }
