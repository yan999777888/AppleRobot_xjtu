...
int main(int argc, char *argv[]) {
  setlocale(LC_ALL,"");
  ros::init(argc,argv,"pick_left_2");
  ros::NodeHandle nh;
  ...
  Eigen::Matrix4d T_cam_to_base = loadMatrix("src/rokae/src/camera_pose_left.txt"); //手眼标定得到的矩阵
  float depth_scale = loadFloat("src/rokae/src/camera_depth_scale_left.txt");
  ...
  try {
    ...
    MoveJCommand moveJ_forward({0.65, 0.2, 0.0, -M_PI / 2, M_PI / 2, -M_PI / 2}); //末端朝前
    ...
    auto doReq = [&](task_assign::Target::Request& req, task_assign::Target::Response& resp) -> bool {
      std::cout << "left Camera coordinates1: (" << req.x << ", " << req.y << ", " << req.z << ")" << std::endl;
      ...
      // 输出结果
      std::cout << "left Camera coordinates2: (" << p_camera(0) << ", " << p_camera(1) << ", " << p_camera(2) << ")" << std::endl;
      std::cout << "Base coordinates: (" << p_base(0) << ", " << p_base(1) << ", " << p_base(2) << ")" << std::endl;
      ...
      // double C = 0; //目标姿态X
      double A = -M_PI / 2, B = M_PI / 2, C = -M_PI / 2;
    ...
    ros::ServiceServer server = nh.advertiseService<task_assign::Target::Request, task_assign::Target::Response>("left_pickTarget",doReq);
    ros::spin();
